...
subroutine step_two_layer(input, output)
    ...
    real(8), dimension(nx+nx*ny), intent(in) :: input
    real(8), dimension(nx+nx*ny), intent(out) :: output
    real(8), dimension(nx+nx*ny) :: k1, k2, k3, k4
...
subroutine step_three_layer(input, output)
    use params
    ...
    ! 4th order Runge-Kutta
    k1 = ode_three_layer(input)
    k2 = ode_three_layer(input+0.5_8*dt*k1)
    k3 = ode_three_layer(input+0.5_8*dt*k2)
    k4 = ode_three_layer(input+dt*k3)
